...
#calculated new consumption based on growth for 2025
Consumption25 = zeros(nSITES)
for i = 1:nSITES
    Consumption25[i] = Consumption[i]*exp(ConsumpGrowth[i]*0.01*(2025-2019))
end
#adjust based on the Export at Trinidad
LNGexport = sum(Production) - sum(Consumption25)
...
@constraint(m, [i=1:nSITES, j=1:nSITES], sum(Ship[j,i]-Ship[i,j]) == Production[j] - Consumption25[j])
